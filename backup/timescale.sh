#!/bin/bash
LOG_TAG="TIMESCALE_BACKUP_SCRIPT"
...
ARCHIVE_FILE="$BACKUP_DIR/timescale-backup-$TIMESTAMP.tar"
log "info" "Starting TimescaleDB backup from $PG_HOST:$PG_PORT"
pg_basebackup -P -R -X stream -c fast -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -D "$TEMP_BACKUP_DIR"
if [ $? -ne 0 ]; then
    log "err" "Failed to create TimescaleDB base backup at: $TEMP_BACKUP_DIR"
    exit 1
fi
log "info" "TimescaleDB base backup created: $TEMP_BACKUP_DIR"
